...
const Navbar = () => {
    ...
    const { isAuthenticated, screenWidth } = useContext(ContextAPI)
            ...
            {screenWidth > 1000 ? (
                <div className='w-full  py-1 flex justify-between items-center'>
                    ...
                    <span className=' flex justify-evenly items-center gap-x-8 '>
                        ...
                        {isAuthenticated ? (
                            <span className='p-3 w-10 cursor-pointer h-10 rounded-full bg-orangeBG text-white'><FaUser /></span>
                        ) : (
                ...
                <>
                    ...
                    <div className={`fixed px-10 py-10 w-full h-screen top-0  z-50 bg-darkBG  left-0 ${menuOpen ? 'translate-y-0' : 'translate-y-full '} transition-all duration-300`}>
                        ...
                        <CustomButton className={'absolute bottom-10 left-1/2 -translate-x-1/2'} text={"Go To Admin"}/>
                    </div>
                </>
            )}
        </div>
